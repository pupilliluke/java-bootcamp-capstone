...
| A4 | The sidebar **Contacts** and **Reports** pages render hardcoded mock data (`MOCK_CONTACTS`, `MOCK_REPORTS`) with no backend behind them — there is no contacts entity or reports source to read. A panelist who clicks either sees fabricated rows. (The Activities page was rewired to live data in #111 and is not affected; seeding, #106, does not touch Contacts/Reports.) | Accepted for the defense, behind the "Demo data" badge | Both screens already carry the orange "Demo data" badge, so the mock is labeled rather than passed off as real, and building real Contacts/Reports is out of scope for the week. The live paths — the Activities feed and the customer Activities tab — use real interactions. If a cleaner demo is wanted, remove the two sidebar links instead. Tracked in issue #102. | Luke Pupilli | 2026-08-27 |
